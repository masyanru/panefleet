...
/// writes the PNG to a sibling `<video>.thumb.png`. Returns the thumbnail path;
/// the caller owns cleanup of both the video and the thumbnail.
///
...
pub async fn generate_video_thumbnail(video: &Path) -> Result<PathBuf, RecordingError> {
    #[cfg(any(macos, linux))]
    {
        thumbnail::generate_video_thumbnail(video, thumbnail::DEFAULT_THUMBNAIL_MAX_WIDTH).await
    }
    #[cfg(not(any(macos, linux)))]
    {
        let _ = video;
