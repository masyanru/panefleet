...
//! active card's lifetime within the session surface through
//! [`BlockingInputSource`], and applies session-specific outcomes such as
//! restoring input or persisting the completed card into the transcript.
...
use super::{BlockingInputSource, TuiTerminalSessionView};
use crate::handoff::{
...
impl TuiTerminalSessionView {
    ...
    pub(super) fn start_handoff(&mut self, argument: Option<&String>, ctx: &mut ViewContext<Self>) {
        ...
        self.blocking_input_source = Some(BlockingInputSource::Handoff(handoff_block));
