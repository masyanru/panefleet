...
- секцию `Current activity` из текущих структурированных состояний сессий;
...
Dashboard не показывает выдуманные progress percentages, skills, tool calls,
подагентов или исторические события. Эти данные появятся только после введения
локального event store и CLI-specific adapters. До этого `Current activity`
является честной проекцией текущего `CLIAgentSessionsModel`.
